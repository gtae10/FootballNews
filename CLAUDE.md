...
**작업을 시작하기 전에 `docs/FEATURE_STATUS.md`를 먼저 읽는다.** 여러 세션에 걸쳐 기능이 개발되면서, 새 세션이 이전 세션의 작업 여부를 몰라 이미 완료된 기능을 중복으로 다시 요청받는 일이 반복되어 왔다. 이 문서는 기능별 구현 상태(코드/DB 반영/테스트/커밋 여부)를 실제로 대조 확인한 사실만 기록한다 — 특히 코드는 있지만 아직 커밋되지 않은 기능은 `git log`만으로는 안 보이므로, 뭔가를 "미구현"이라고 판단하기 전에 이 문서와 working tree(`git status`)를 함께 확인한다.
...
- **기능을 완료하면 `docs/FEATURE_STATUS.md`도 함께 갱신한다.** 상태(✅/⚠️/❌), 코드/DB 반영 여부, 테스트 통과 여부, 확인 날짜(커밋 해시 또는 "미커밋")를 실제로 확인한 사실 그대로 적는다 — 추측이나 계획은 적지 않는다.
...
- **기능 구현 현황(새 세션 시작 시 필수): `docs/FEATURE_STATUS.md`**
