...
**해결된 과거 이슈(2026-09-13)**: 한때 `source_tier=1`이 최우선이라, 본문이
영구히 비어 있어 매번 실패하는 tier-1 기사(예: 만료된 Sky Sports 라이브
블로그)가 매 배치 큐 맨 앞에서 재시도되는 문제가 있었다(API 호출 전에 빈 본문
가드가 막아 비용은 안 들지만 한도 슬롯을 일부 차지, "번역 실패" 로그 줄 증가).
`collector/db.py`의 `save_articles()`가 저장 시점에 본문/og:description 폴백을
시도하고 그래도 실패하면 저장 자체를 건너뛰도록 바뀌면서, 빈 `content_original`
기사 자체가 더 이상 이 DB에 쌓이지 않는다 — 근본 원인이 사라져 우선순위 큐
쪽에는 별도 수정을 하지 않았다(`docs/ARCHITECTURE.md`의 번역 파이프라인 섹션
참고).
