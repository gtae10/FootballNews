...
- RSS의 `<description>`이 비어 있는 기사(주로 Sky Sports Football 라이브 블로그)는
  저장 전에 원문 페이지에서 본문 확보를 시도한다: 본문 텍스트 크롤링
  (`collector/body_text_extractor.py`) → 실패하면 `og:description` 메타 태그 →
  그래도 실패하면 저장 자체를 건너뛴다(빈 본문 기사가 DB에 쌓이지 않는다). 이
  페이지 접속은 본문 이미지 크롤링(`collector/body_image_extractor.py`)과
  공유해 기사당 요청을 한 번만 보낸다. 이용약관상 개별 기사 페이지 크롤링이
  금지된 도메인(현재 empireofthekop.com — `sources.py` 주석 참고)은 이 폴백
  자체를 시도하지 않고 RSS 피드만 계속 사용한다
...
- **우선순위 큐 + 하루 처리 한도**: `status='COLLECTED'`인 기사는 사실상 번역
  ...
  - **해결된 과거 이슈(2026-09-13)**: 한때 `source_tier=1` 기사가 우선순위
    1순위라, 본문이 영구히 비어 있는 tier-1 기사(만료된 라이브 블로그 등)가
    매 배치마다 큐 맨 앞에서 재시도되는 문제가 있었다(API 비용은 안 들지만
    한도 슬롯을 일부 차지). 이를 우선순위 큐 쪽에서 "실패 이력으로 후순위로
    미루는" 로직으로 고칠지 검토했으나, 애초에 위 수집 스케줄러 섹션의 본문/
    og:description 폴백 도입으로 **빈 `content_original` 기사 자체가 더 이상
    저장되지 않게(또는 스킵되게) 되어** 이 문제의 근본 원인이 사라졌다 —
    별도의 우선순위 큐 수정은 필요 없다고 판단해 추가하지 않았다.
