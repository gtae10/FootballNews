...
### 22. 채팅(챗봇) — RAG 기반 3단계 답변 구조

**⚠️ 코드/테스트 완료, 미커밋** (2026-09-14)

- 배경: 챗봇의 기본 RAG 버전(DB에서 관련 기사를 찾아 그 내용에만 근거해 답하고, 못 찾으면
  "관련된 기사를 찾지 못했습니다"라고만 답함)은 이전 세션에서 이미 구현돼 있었으나(미커밋),
  이 문서에 기록된 적이 없었음. 이번 세션에서 DB에 없는 질문(선수 배경, 역대 기록, 일반
  축구 규칙 등)도 LLM 일반 지식으로 보완하되 출처 종류를 항상 구분 표시하도록 확장 요청받음.
- 코드:
  - `backend/.../service/ChatService.java`: 시스템 프롬프트를 3단계로 확장 — (1) DB
    키워드 검색(기존 로직 그대로) → (2) 관련 기사를 찾으면 `[저장된 기사 기반]` 표시 +
    출처/날짜/신뢰도 등급 명시 → (3) 못 찾았거나 검색된 기사가 질문과 실제로 무관하면
    `[AI 일반 지식 — 최신 정보 아닐 수 있음]` 표시로 LLM 일반 지식 답변. 시점이 중요한
    사실(최근 이적/영입/방출)은 불확실성을 명시하고, 역대 기록/규칙 같은 시점 무관 질문엔
    비교적 자신 있게 답하되 표시는 항상 유지하도록 지시.
  - `ChatResponse`에 `sourceType`(`"ARTICLE"`/`"GENERAL_KNOWLEDGE"`) 필드 신규 추가 —
    아래 "라이브 테스트 버그 2" 때문에 `matchedArticleCount` 대신 이 필드로 프론트 태그
    색을 결정하게 함.
  - `frontend-web/.../components/ChatWidget.jsx`/`theme.css`: 답변 말풍선 위에 출처
    태그(틸/청록 계열 "저장된 기사 기반" vs 회색 계열 "AI 일반 지식") 표시, 기존 신뢰도
    등급 뱃지 색(T1 초록/T2 파랑/T5 갈색)과 겹치지 않는 색상으로 선정.
- **라이브 테스트로 실제 발견한 버그 1 — OpenAI 응답 토큰 부족**: `gpt-5-mini`는 추론
  토큰(reasoning tokens)을 completion 예산에서 함께 소비하는데, 기존 기본값
  (`CHAT_MAX_TOKENS=500`)으로는 긴 시스템 프롬프트 + 다건 기사 컨텍스트가 있는 질문에서
  추론 토큰만으로 예산이 소진돼 실제 답변(`content`)이 빈 문자열로 오는 경우가 실제로
  재현됨(503 "OpenAI 응답 본문이 비어 있습니다"). `application.yml`의
  `app.chat.max-tokens` 기본값을 500 → **2500**으로 올려 해결 확인(기사 5건이 컨텍스트에
  포함된 질문까지 재현 테스트로 확인).
- **라이브 테스트로 실제 발견한 버그 2 — 키워드 검색 오탐 시 출처 태그 신뢰성**:
  "1966년 월드컵 우승팀이 어디야?"처럼 DB에 없는 일반 지식 질문인데도, 키워드 매칭이
  무관한 기사(당시 진행 중이던 월드컵 관련 기사 등) 5건을 걸어온 사례가 실제로 발생함.
  모델은 그 기사들을 무시하고 정확한 일반 지식으로 답했지만, `matchedArticleCount > 0`
  이라는 이유만으로 프론트 태그를 "저장된 기사 기반"으로 표시하면 실제 답변 내용과 태그가
  어긋나는 문제가 있었음. 수정: `ChatService`가 모델이 답변 맨 앞에 실제로 붙인 표시를
  우선 파싱해 `sourceType`을 결정하고, 표시가 없을 때만 `matchedArticleCount`로 보수적
  추정하도록 변경. 시스템 프롬프트에도 "검색 결과가 있어도 질문과 무관하면 3번(일반 지식)
  규칙을 따르라"는 지시를 명시적으로 추가.
- 라이브 검증(2026-09-14, 실제 OpenAI API 키로 로컬 backend 구동 후 curl로 직접 확인):
  1. DB에 있는 최신 기사 질문("홀란드가 맨유전에서 넣은 골 얘기해줘") →
     `sourceType: "ARTICLE"`, `[저장된 기사 기반]` 표시 + 기사별 출처/시간/신뢰도 등급
     인용, 5건 근거로 정리된 답변 확인
  2. DB에 없는 일반 지식 질문("1966년 월드컵 우승팀이 어디야?", 키워드 오탐으로 무관
     기사 5건이 컨텍스트에 섞인 상태) → `sourceType: "GENERAL_KNOWLEDGE"`,
     `[AI 일반 지식 — 최신 정보 아닐 수 있음]` 표시로 정확한 답변(잉글랜드/제프 허스트
     해트트릭) 확인
  3. DB에 없는 "최신 이적" 질문("손흥민이 최근 토트넘을 떠났나요?") →
     `sourceType: "GENERAL_KNOWLEDGE"`, "제가 알기로는(2024년 6월 기준) ... 확실하지
     않습니다"처럼 확정적으로 말하지 않고 불확실성을 명시하는 것 확인, 답변 끝에 앱 내
     기사 확인 안내 포함
- 브라우저 UI 확인: 프론트 dev 서버(`npm run dev`)로 게스트 피드까지는 정상 확인했으나,
  이 세션에 연결된 Chrome 브라우저에서 플로팅 채팅 버튼 자체가 DOM에 나타나지 않는 현상이
  있었음. 서버가 내려주는 번들에는 `ChatWidget`이 정상 포함돼 있음을 `curl`로 직접
  확인했고, 같은 페이지의 다른 버튼들(제3자 확장 프로그램이 주입한 버튼 포함)은 정상
  렌더링됨 — 이 브라우저 세션의 확장 프로그램 충돌로 추정, 코드 결함으로 보지 않음(다른
  프로필/시크릿 창에서 재확인 필요, 다음 세션 숙제로 남김). 태그 렌더링 자체는
  `ChatWidget.test.jsx`(vitest, jsdom)로 두 케이스 모두 확인함.
- 테스트: 백엔드 `ChatServiceTest`(+4: 시스템 프롬프트 표시 규칙 포함 여부, 출처 태그
  파싱 성공/폴백 케이스), `ChatControllerTest`(sourceType 필드 반영) — 백엔드 전체 79개
  통과. 프론트 `ChatWidget.test.jsx`(+2: 두 출처 태그 렌더링) — 프론트 전체 60개 통과.
- 문서: `docs/API.md`에 `POST /chat` 섹션 신규 추가(이전 세션에서 기능만 구현되고 문서화가
  누락돼 있었음).
- 미해결: 위 "브라우저 UI 확인" 항목(다른 브라우저 프로필로 재확인 필요). 커밋 전 diff
  재검토 필요 — git status상 이번 세션에서 손대지 않은 백엔드 파일(`SecurityConfig`,
  `GlobalExceptionHandler`, `ArticleRepository` 등)도 이미 수정된 상태로 있었음(이전
  세션 작업분으로 추정, 이번 세션에서 내용 확인하지 않음).
